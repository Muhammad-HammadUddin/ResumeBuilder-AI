  ...
  return (
    <div>
        <div className='max-w-7xl mx-auto px-4 py-8'>
            ...
            mb-6 bg-gradient-to-r from-slate-600 to-slate-700 bg-clip-text text-transparent sm:hidden 
            '>Welcome,Joe Doe</p>
            ...
            <div className='flex gap-4'>
                <button onClick={()=>setShowCreateResume(true)} className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 text-slate-600 border border-dashed border-slate-300 group hover:border-purple-500 hover:shadow-lg transition-all duration-300 cursor-pointer'>
                    ...
                    p-2.5 bg-gradient-to-br from-indigo-300 to-indigo-500 text-white rounded-full
                ...
                <button onClick={()=>setShowUploadResume(true)} className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 text-slate-600 border border-dashed border-slate-300 group hover:border-purple-500 hover:shadow-lg transition-all duration-300 cursor-pointer'>
                    ...
                    p-2.5 bg-gradient-to-br from-purple-300 to-purple-500 text-white rounded-full
  ...
  {allResumes.map((resume, index) => {
    ...
    return (
      ...
      onClick={()=>navigate(`/app/builder/${resume._id}`)}
        key={index}
        className="relative w-full sm:w-36 h-48
        flex flex-col items-center justify-center
        rounded-lg gap-2 border
        group hover:shadow-lg transition-all duration-300"
        style={{
          background: `linear-gradient(135deg, ${baseColor}40)`,
      ...
      >
        ...
        {/* TITLE */}
        <p className="text-sm text-center px-2 group-hover:scale-105 transition-all">
          {resume.title}
        </p>
        ...
        <div onClick={e=>e.stopPropagation()} className="absolute top-1 right-1 hidden group-hover:flex items-center gap-1">
          <TrashIcon className="size-7 p-1.5 rounded hover:bg-white/50 transition-colors"   onClick={()=>deleteResume(resume._id)} />
          <PencilIcon onClick={()=>{setEditResumeId(resume._id);setTitle(resume.title)}} className="size-7 p-1.5 rounded hover:bg-white/50 transition-colors" />
        </div>
      </button>
    )
  })}
          ...
          {showCreateResume&&(
            ...
            onClick={()=>setShowCreateResume(false)}className="fixed inset-0 bg-black/70 backdrop-blue bg-opacity-50
            z-10 flex items-center justify-center
            ">
                <div onClick={e=>e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
                    <h2 className='text-xl font-blod mb-4'>
                        Create a Resume
                        <input 
          ...
          {showUploadResume && 
          (
            <form onSubmit={e=>uploadResume(e)} onClick={()=>setShowUploadResume(false)}className="fixed inset-0 bg-black/70 backdrop-blue bg-opacity-50
            z-10 flex items-center justify-center
            ">
                <div onClick={e=>e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
                    <h2 className='text-xl font-blod mb-4'>
           ...
           {editResumeId&&(
            ...
            onClick={()=>setEditResumeId('')}className="fixed inset-0 bg-black/70 backdrop-blue bg-opacity-50
            z-10 flex items-center justify-center
            ">
                <div onClick={e=>e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
                    <h2 className='text-xl font-blod mb-4'>
