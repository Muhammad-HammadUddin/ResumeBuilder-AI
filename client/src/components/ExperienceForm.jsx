  ...
  return (
    <div className='space-y-6'>
        ...
        <div>
            ...
            <div>
<h3 className='fle items-center gap-3 text-lg font-semibold tet-gray-900'>Professional Exerience</h3>
<p>Add Summary for your job Experience</p>
            </div>
            <button onClick={addExperience} className='flex items-center gap-2 px-3 py-1 text-sm bg-green-10 text-green-700 rounded-lg hover:bg-green-200 transition-colors '>
                        ...
                        {data?.map((experience,index)=>(
                             <div key={index} className='p-4 border border-gray-200 rounded-lg space-y-3'>
                                ...
                                <label htmlFor="" className='flex items-center gap-2'>
                                    <input type="checkbox" name="" id="" checked={experience?.is_current||false} onChange={(e)=>updateExperience(index,"is_current",e.target.checked?true:false)} className='rounded border-gray-300 text0blue-600 focus:ring-blue-500'/>
                                    <span className='text-sm text-gray-700'>Currently working here </span>
                                </label>
                                <div className='space-y-2'>
                                    <div className='flex items-center justify-between'>
                                        <label htmlFor="" className='text-sm font-medium text-gray-700 '>job Description</label>
                                        <button onClick={()=>generateDescription(index)} className='flex items-center gap-1 px-2 py-1 text-xs bg-purple-100 text-purple-700 rounded hover:bg-purple-200 transition-colors disabled:opacity-50'
